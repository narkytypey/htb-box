...
**Source spec:** `$REPO/donerup-htb-insane-design-v2.md` (v3), §10 and §11 (and the full chain map in §2).
...
LDAP_BIND_PASSWORD=KebapciBind2026!Sec
```
...
The password in the copied file must match whatever `svc_ldap`'s password actually is on the Plan 3 DC (`KebapciBind2026!Sec` if Plan 3's Task 2 script was used unmodified).
...
docker compose -f ../docker-compose.yml exec -T legacy-auth-db \
    mysql -uroot -p'DonerciKral99!' -e "SELECT username FROM legacy_auth.users;" > /tmp/replay-rabbithole.out 2>&1
...
cd build/exploit
./full-chain-replay.sh localhost:8080 10.10.20.10 'KebapciBind2026!Sec'
```
...
- [x] **Step 1: SSH (22) is a genuine dead end — no AD/service credential works**

```bash
for cred in "administrator:R00tP@ssw0rd2026!" "svc_ldap:KebapciBind2026!Sec" "jdoe:SogukDonerAyran7"; do
  user="${cred%%:*}"; pass="${cred##*:}"
...
Expected: `PASS: ... rejected over SSH` for all three; a `FAIL: ... (unexpected result: ...)` would indicate a genuine credential-reuse problem, and `SKIP: sshd not reachable ...` means the check could not be made at all.

**Result (2026-08-28):** all three `PASS`. This is a real rejection, not an artefact of an unreachable or key-only sshd: `sshd -T` on the host reports `passwordauthentication yes`, so the daemon actually evaluated each password and refused it. (An earlier revision of this plan expected `SKIP` here, from a time when sshd was not listening on the docker host.)

- [x] **Step 2: Tunnel DNS resolution for `donerup.htb`/`dc01` is documented and works**

```bash
certipy find -u svc_ldap -p 'KebapciBind2026!Sec' -dc-ip 10.10.20.10 -ns 10.10.20.10 -stdout > /tmp/replay-dns-check.out 2>&1
...
Expected: `donerup-ad-pivot.service` shows `active (exited)` without being re-enabled by hand and the integration smoke test still prints `ALL INTEGRATION CHECKS PASSED` — confirming `After=docker.service` ordering (Plan 2 Task 3) survives a real reboot.

Note the isolation test is **not** part of this expectation on a host where Plan 3 has run. `run_isolation_test.sh` retires itself once the real AD VLAN exists (`host address 10.10.20.1 is already assigned -- adlab0 has been provisioned (Plan 3 has run)`) rather than fabricating a namespace on an address the lab now genuinely owns, and points at this plan's replay instead. An earlier revision of this step expected `ALL ISOLATION CHECKS PASSED` here, which that guard makes unreachable.

**Result (2026-08-28):** PASS. The unit was missing on the host entirely — Plan 2 Task 3 Step 5 had never been run here — so it was installed first, then the host was rebooted. Boot at 17:33:13, `donerup-ad-pivot.service` `active (exited)` at 17:33:23 with all four chain rules present, no failed units, no manual step. Containers did not return on their own (the compose file sets no restart policy, which is expected); after `docker compose up -d` the smoke test printed `ALL INTEGRATION CHECKS PASSED`. `docker compose down` + `up -d` also leaves the chain intact.

**Still open after this step:** the AD-VLAN isolation control itself is currently verified by neither path on this lab. `run_isolation_test.sh` has retired itself, and the replay's Phase 0 reports SKIP because the attacker box sits on the AD VLAN. It needs one run from a VPN-side host.

- [x] **Step 4: Record the results**

No commit for this task — its only output is the **Result** blocks recorded under each step above and the Self-Review entries below. If any check fails, fix the underlying plan (most likely Plan 2's systemd unit for Step 3, or host `sshd_config` for Step 1) and re-run before considering Plan 4 complete.
